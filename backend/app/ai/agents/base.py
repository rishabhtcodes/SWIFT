...
class BaseAgent(ABC):
    ...
    async def think(self, state: GraphState, session: AsyncSession) -> dict[str, Any]:
        """Core reasoning step: retrieve memory, call LLM, decide next action."""
        ...
        print("DEBUG MESSAGES:", messages, flush=True)
        ...
        return {"content": response.content, "model": response.model_id, "tokens": (response.tokens_in, response.tokens_out)}
